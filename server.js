...
const analisisTxt = analisis.analisis || "";
const gestionRec  = analisis.gestion_recomendada || {};

const emojiH  = cal >= 9 ? "🔥" : "🟡";
const estado  = cal >= 9 ? "EXCEPCIONAL" : "VALIDO";
const emojiDir = direccion === "LONG" ?
    "🟢 LONG" : "🔴 SHORT";

const check = (val) =>
    String(val).includes("Confirmado") ? "✅" : "⚠️";

const prof = evento.profundidad_spring ||
             evento.profundidad_upthrust || "";
const velasEv = evento.velas_desde_spring ||
                evento.velas_desde_upthrust || "";
const toques = rango.toques_soporte ||
               rango.toques_resistencia || "";
...
const mensaje =
    ...
    `─────────────────────────────\n` +
    `TRADE:\n` +
    `Entrada:  ${gestionRec.entrada || gestion.entrada || ""}\n` +
    `Stop:     ${gestionRec.stop_loss || gestion.stop_loss || ""} 🔴\n` +
    `Target 1: ${gestionRec.target_1 || gestion.target_1 || ""} 🎯 (${gestionRec.porcentaje_parcial || 60}%)\n` +
    `Target 2: ${gestionRec.target_2 || gestion.target_2 || ""} 🎯\n` +
    `RR:       1:${gestion.RR || ""}\n` +
    `Mover BE: ${gestionRec.mover_be_en || gestion.zona_BE || ""}\n` +
    `─────────────────────────────\n` +
    `ANALISIS IA:\n` +
    `${analisisTxt}\n` +
    `─────────────────────────────\n` +
...
try {
    ...
    if (calificacion >= CALIFICACION_MINIMA) {
        ...
        console.log(
            `Senal enviada: ${trade} ${calificacion}/10`
        );
        ...
        console.log(
            `Rechazado: ${trade} ${calificacion}/10`
        );
